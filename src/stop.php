...
// ---------------------------------------------------------------------------
$mysqli->execute_query('CALL DixitStopGame(?, ?, ?);', [$username, $pwd_hash, $gameId]);
if ($mysqli->affected_rows != 1) {
	http_response_code(403);
	exit();
}
$mysqli->execute_query('CALL DixitResetDeck(?, ?, ?);', [$username, $pwd_hash, $gameId]);
if ($mysqli->affected_rows != 84) {
	http_response_code(403);
	exit();
}
$mysqli->execute_query('CALL DixitResetVotes(?, ?, ?);', [$username, $pwd_hash, $gameId]);
